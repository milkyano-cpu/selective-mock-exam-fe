...
import { Plus, Edit2, Trash2, BookOpen, ChevronRight, Search, Loader2 } from 'lucide-react';
import Link from 'next/link';

export default function SubjectsPage() {
  const { subjects, meta, isLoading, fetchSubjects, createSubject, updateSubject, deleteSubject } = useSubjects();
  ...
  const onSubmitSubject = async (data: { name: string; description?: string | null }) => {
    let success = false;
    if (selectedSubject) {
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 dark:text-slate-100 flex items-center gap-2">
            <BookOpen className="text-[#0A9AE2]" size={24} />
            Subjects
          </h1>
          <p className="text-sm font-medium text-slate-500 dark:text-slate-400 mt-1">
            Manage all educational subjects and their topics
          </p>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm text-slate-600 dark:text-slate-300">
            <thead className="bg-slate-50 text-slate-900 dark:bg-slate-800 dark:text-slate-100 border-b border-slate-200 dark:border-slate-700">
              <tr>
                <th className="px-6 py-4 font-bold rounded-tl-xl">Subject Name</th>
                <th className="px-6 py-4 font-bold">Description</th>
                <th className="px-6 py-4 font-bold text-center">Topics Count</th>
            ...
            <tbody>
              {isLoading && subjects.length === 0 ? (
                ...
                subjects.map((subject) => (
                  <tr key={subject.id} className="border-b border-slate-100 last:border-0 hover:bg-slate-50 transition-colors dark:border-slate-800 dark:hover:bg-slate-800/50">
                    <td className="px-6 py-4 font-bold text-slate-900 dark:text-slate-100">
                      {subject.name}
                    </td>
                    <td className="px-6 py-4 max-w-xs truncate">
                      {subject.description || <span className="text-slate-400 italic">No description</span>}
                    </td>
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <Link
                          ...
                          className="p-2 text-slate-400 hover:text-[#0A9AE2] hover:bg-blue-50 rounded-xl transition-colors dark:hover:bg-blue-500/10"
                          title="View Topics"
                        >
