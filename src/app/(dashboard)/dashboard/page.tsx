...
function MonthlyRegistrationsSkeleton() {
  const barHeights = [55, 78, 42, 88, 64, 72];
  return (
    <div className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
      <div className="mb-4 animate-pulse">
        <div className="h-5 w-44 rounded-lg bg-slate-200/70 dark:bg-slate-800" />
        <div className="mt-2 h-3 w-64 rounded-lg bg-slate-100 dark:bg-slate-800/60" />
      </div>
      <div className="flex h-[220px] items-end gap-3 px-2 pb-6 pt-2 animate-pulse">
        {barHeights.map((h, i) => (
          <div key={i} className="flex flex-1 items-end gap-1">
            <div
              className="flex-1 rounded-t-md bg-blue-200/70 dark:bg-blue-900/40"
              style={{ height: `${h}%` }}
            />
            <div
              className="flex-1 rounded-t-md bg-pink-200/70 dark:bg-pink-900/40"
